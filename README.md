...
## Playwright / Screenshot Sorun Giderme

`mcp__browser_tools__run_playwright_script` kullanırken `net::ERR_EMPTY_RESPONSE` alırsan şu adımları uygula:

1. Eski server süreçlerini kapat:

```bash
pkill -f "http.server 4173" || true
```

2. Server'ı açıkça dış arayüze bind ederek başlat:

```bash
python -m http.server 4173 --bind 0.0.0.0 --directory /workspace/Match-
```

3. Önce HTTP yanıtını doğrula:

```bash
curl -I http://127.0.0.1:4173
```

> Not: Bazı CI/container ortamlarında MCP browser ağı, çalışma konteynerindeki localhost servisine erişemeyebilir. Bu durumda uygulama kodu doğru olsa bile Playwright tarafında `ERR_EMPTY_RESPONSE` görülebilir.
...
### Türkçe Kelime Gelişim Modu

Sohbet içinde Türkçe kelime dağarcığını güçlendirmek için özel komutlar bulunur:

- `10 kelime ver` → anlam + eş anlam + zıt anlam + örnek cümle
- `İleri seviye kelime` → akademik/soyut kelime seti
- `Kelime testi` → cümle kurma odaklı mini alıştırma
- `Sıradaki kelime` → tek tek kelime ilerleme akışı
- `Kelime durumu` → görülen/doğru/yanlış istatistik özeti
- `Doğru` / `Yanlış` → son kelimeyi bildim-bilemedim işaretleme
- `Kelime sıfırla` → ilerleme sayaçlarını temizleyip yeniden başlatma
- `Profilimi göster` → kayıtlı oturum özetini gösterir
- `Haftalık plan` → profile göre otomatik çalışma planı üretir
- `Günlük görev` → günlük checklist üretir ve ilerleme takibi sağlar
- `Görev 1 tamam` → görev tamamlama işaretleme
- `Görev sıfırla` → günlük checklisti yeniler
- `İlerleme özeti` → görev + kelime + seri bilgisini özetler
- `Sıradaki öneri` → mevcut duruma göre en iyi sonraki adımı önerir
- `Zor kelimeleri tekrar et` → hatalı/eksik kelimelerden tekrar listesi üretir
- `Günü kapat` → en az 1 görev sonrası günlük seriyi işler
- `Oturumu sıfırla` → profil + kelime ilerlemesini temizler
- `Şu ana kadar ne yaptık` → oturumun kısa özetini çıkarır
- `Yardım` → tüm komutların kısa listesini gösterir
- `Cevabı göster` → aktif quiz veya kelime testinin örnek yanıtı
